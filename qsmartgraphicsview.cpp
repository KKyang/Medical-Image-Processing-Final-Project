...
void QSmartGraphicsView::setImage(const cv::Mat &img)
{
    ...
    QImage img_temp = QImage((uchar*)tmp.data, tmp.cols, tmp.rows, tmp.step, QImage::Format_RGB888);
...
void QSmartGraphicsView::setImage(const std::vector<cv::Mat> &imgs)
{
    ...
    for(size_t i = 0; i < imgs.size(); ++i){
        ...
        QImage img_temp(tmp.cols, tmp.rows, QImage::Format_RGB888);
        for(int y = 0; y < tmp.rows; ++y){
            memcpy(img_temp.scanLine(y), tmp.data + y * tmp.cols * 3, tmp.cols * 3);
        }
