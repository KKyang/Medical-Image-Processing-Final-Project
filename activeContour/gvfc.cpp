...
float cvFindOpElem(const CvArr* srcarr,
                           int flag)
{
...
//    CV_CALL( dst_v = cvGetMat(dstarr_v, &dstubv ));
    ...
    src = cvGetMat(srcarr, &sstub );
    dst_u = cvGetMat(dstarr_u, &dstubu );
    dst_v = cvGetMat(dstarr_v, &dstubv );
    size = cvGetSize(src);
                  ...
                  int calcInitial)
{
    ...
    u = cvGetMat(src_u, &sstub_u );
    //CV_CALL( v = cvGetMat(src_v, &sstub_v ));
    v = cvGetMat(src_v, &sstub_v );
    roi = cvGetSize( u );
